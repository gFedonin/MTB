...
from data_reading import read_pheno, read_snp_list, read_subset, read_variants
...
def process_snps(sample_id, var_list):
    res = set()
    deletions = {}
    broken_genes = set()
    for var in var_list:
...
def main():
    ...
    if use_extended_features:
        tasks1 = parallel(delayed(process_snps)(sample_id, snp_list) for sample_id, snp_list in tasks)
        tasks = tasks1
    for name, l in tasks:
    ...
    with open(out_path, 'a') as f:
        ...
        f.write('LR_l1 C1 extended feature set filtered mc3 indels\n\n')
