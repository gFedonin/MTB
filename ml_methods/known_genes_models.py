...
from src.core.data_reading import read_pheno, read_snp_list, read_subset, read_variants

path_to_pheno = '../../data/pheno_mc5/'
path_to_snp = '../../data/snps/annotated_with_pheno_and_snp_mc5_snp_only/'
path_to_snp_list = path_to_snp + 'dr_genes_var_list.csv'
...
def stat_cv_multi(X, y, clf):
    ...
    if tn + fp > 0:
        ...
        specificity = 0
    return ppv, npv, sensitivity, specificity, cv_results['f1'].mean(), cv_results['auc'].mean()
...
def drug_stat_multi(pheno, X, sample_id_to_index, clf, subset=None):
    ...
    samples_with_pheno = len(sample_indexes)
    if samples_with_pheno == 0 or sum == len(y) or sum == 0:
        return samples_with_pheno, 0, 0, 0, 0, 0, 0
    else:
        if subset is None:
            X_slice = X[sample_indexes, :]
            ppv, npv, sensitivity, specificity, f1, auc = stat_cv_multi(X_slice, y, clf)
        else:
            ppv, npv, sensitivity, specificity, f1, auc = stat_fixed_multi(X, sample_indexes, y, subset_indices, clf)
        return samples_with_pheno, ppv, npv, sensitivity, specificity, f1, auc
...
def print_stat_multi(f, drug_to_pheno, X, sample_id_to_index, clf):
    ...
    for drug, pheno in drug_to_pheno.items():
        f.write(drug)
        samples_with_pheno, ppv, npv, sensitivity, specificity, f1, auc = drug_stat_multi(pheno, X, sample_id_to_index, clf)
...
def print_stat_fixed_multi(f, drug_to_pheno, X, sample_id_to_index, clf, subset):
    ...
    for drug, pheno in drug_to_pheno.items():
        f.write(drug)
        samples_with_pheno, ppv, npv, sensitivity, specificity, f1, auc = drug_stat_multi(pheno, X, sample_id_to_index, clf, subset)
        f.write('\t%d\t%1.2f\t%1.2f\t%1.2f\t%1.2f\t%1.2f\t%1.2f\n' % (samples_with_pheno, ppv, npv, sensitivity, specificity, f1, auc))
