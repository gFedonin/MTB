...
from src.core.constants import custom_scoring, tp, tn, fp, fn, data_path
from src.core.data_reading import read_pheno, read_dict, read_subset, read_variants
from src.ml_methods.vocabulary_clf import VocabularyClf

path_to_pheno = data_path + 'pheno_mc5_mega/'
path_to_snp = data_path + 'snps/annotated_with_DR_with_indel_with_pheno_and_snp_mc10_long_del/'
...
out_path = '../../res/dict_simple_multi_Walker_train_rest_test_new_new.out'
...
def list_stat_fixed_multi(X, sample_indexes, y, f_list, feature_to_index, subset_indices, clf):
    ...
    if specificity > 0 and sensitivity > 0:
        f1 = 2/(1/sensitivity + 1/specificity)
    else:
        f1 = 0
    return ppv, npv, sensitivity, specificity, f1, roc_auc_score(y_test, y_pred)
...
def drug_stat_fixed_multi(drug, pheno, dict_name_to_list, X, sample_id_to_index, feature_to_index, subset, clf):
    ...
    if samples_with_pheno == 0 or sum == len(y) or sum == 0:
        for dict_name, f_list in dict_name_to_list.items():
            dict_name_to_stat[dict_name] = (0, 0, 0, 0, 0, 0, 0)
    else:
        # print(str(len(sample_indexes)) + ' samples with pheno')
        for dict_name, f_list in dict_name_to_list.items():
            # print(dict_name)
            if drug in f_list.keys():
                features = f_list[drug]
                ppv, npv, sensitivity, specificity, f1, auc = list_stat_fixed_multi(X, sample_indices, y, features,
                                                                           feature_to_index, subset_indices, clf)
                dict_name_to_stat[dict_name] = (len(features), ppv, npv, sensitivity, specificity, f1, auc)
            else:
                dict_name_to_stat[dict_name] = (0, 0, 0, 0, 0, 0, 0)
    return samples_with_pheno, dict_name_to_stat


def print_stat_fixed_multi(f, dict_name_to_list, drug_to_pheno, X, sample_id_to_index, feature_to_index,
                     subset=None, clf=None):
    f.write('Drug\tSamples')
    for name in dict_name_to_list.keys():
        f.write('\t' + name)
    ...
    for drug, pheno in drug_to_pheno.items():
        ...
        f.write('\t' + str(samples_with_pheno))
        for name in dict_name_to_list.keys():
            feature_num, ppv, npv, sensitivity, specificity, f1, auc = dict_name_to_stat[name]
            # try:
            #     f_count = dict_name_to_counts[name][drug]
            # except:
            #     f_count = 0
            f.write('\t%d;%1.2f;%1.2f;%1.2f;%1.2f;%1.2f;%1.2f' % (feature_num, ppv, npv, sensitivity, specificity, f1, auc))
        f.write('\n')
...
def main():
    ...
    sample_to_snps = {}
    tasks = Parallel(n_jobs=-1)(delayed(read_variants)(path_to_snp, sample_id, full_dict_set, True) for sample_id in sample_ids)
    for name, l in tasks:
    ...
    with open(out_path, 'w') as f:
        ...
        f.write('FeatureNum/PPV/NPV/Sensitivity/Specificity/F1/AUC\n\n')
        ...
        clf = VocabularyClf()
        print_func(f, dict_name_to_list, drug_to_pheno, X, sample_id_to_index, feature_to_index,
                   subset=set_name_to_list['Walker_subset'], clf=clf)
        f.write('\n')
        f.write('LogisticRegression l1\n\n')
        clf = LogisticRegression(penalty="l1", dual=False, class_weight='balanced')
        print_func(f, dict_name_to_list, drug_to_pheno, X, sample_id_to_index, feature_to_index,
                   subset=set_name_to_list['Walker_subset'], clf=clf)
        f.write('\n')
        f.write('LinearSVC l2\n\n')
        clf = LinearSVC(penalty='l2', dual=False, class_weight='balanced', C=1)
        print_func(f, dict_name_to_list, drug_to_pheno, X, sample_id_to_index, feature_to_index,
                   subset=set_name_to_list['Walker_subset'], clf=clf)
        f.write('\n')
        f.write('SVM RBF\n\n')
        clf = SVC(class_weight='balanced', C=1)
        print_func(f, dict_name_to_list, drug_to_pheno, X, sample_id_to_index, feature_to_index,
                   subset=set_name_to_list['Walker_subset'], clf=clf)
        f.write('\n')
        f.write('RandomForestClassifier n=1000\n\n')
        clf = RandomForestClassifier(n_estimators=1000, n_jobs=threads, class_weight='balanced')
        print_func(f, dict_name_to_list, drug_to_pheno, X, sample_id_to_index, feature_to_index,
                   subset=set_name_to_list['Walker_subset'],clf=clf)
        f.write('\n')
