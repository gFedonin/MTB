...
def format_snp(sample_id, snps, ref_seq, ref_seq_compl, snp_to_cds):
            ...
            if cds is not None:
                if cds.strand == 1:
                    if cds.type == CDSType.Gene:
                        ...
                        res.append('\t'.join(
                            (cds.type.name, cds.name, str(pos - cds.start), ref_seq[pos - 1], alt)))
                else:  # if strand is '-'
                    if cds.type == CDSType.Gene:
...
def main():
    ...
    formatted_snps = Parallel(n_jobs=-1)(
        delayed(format_snp)(sample_id, snps, h37rv, h37rv_compl, snp_to_cds)
