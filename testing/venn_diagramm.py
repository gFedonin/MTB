...
path_to_drug_codes = '../../data/xparr/drug_codes.txt'
...
def read_epistat():
    ...
    for line in open(path_to_drug_codes).readlines():
        s = line.strip().split('\t')
        drug_codes[s[1]] = s[0]
    drug_to_gene_set = {drug: set() for drug in drug_codes.values()}
    for line in open(path_to_epistat).readlines()[1:]:
        s = line.strip().split('\t')
        name = s[1]
        drug = drug_codes[s[3]]
        if name not in walker_genes:
            gene_set = drug_to_gene_set[drug]
            gene_set.add(name)
    return drug_to_gene_set
...
if __name__ == '__main__':
    ...
    for drug, lr_gene_set in lr_drug_to_gene.items():
        ...
        epistat_gene_set = epistat[drug]
        v = venn3([epistat_gene_set, lr_gene_set, sgicm_gene_set], ('epistat6', 'log_reg', 'SGIMC'))
