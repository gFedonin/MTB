...
from data_reading import read_pheno, read_subset, read_variants
from constants import tp, tn, fp, fn, aminoacids
from mifslr import MIFSLR
from migffslr import MIGFFSLR

path_to_pheno = './data/pheno/'
path_to_snp = './data/snps/annotated_transformed_genes_with_pheno_and_snp_mc5/'
...
def drug_stat_multi(drug, snp_train, snp_test, pheno_train, pheno_test, clf, full_snp_list):
    ...
    for i in range(len(sample_ids)):
        for f in snp_train[sample_ids[i]]:
            ...
            if s[0] == 'Gene':
                f = '\t'.join(s[:-1])
                X_train[i, feature_to_index[f]] = float(s[-1])
            else:
    ...
    for i in range(len(sample_ids)):
        for f in snp_test[sample_ids[i]]:
            ...
            if s[0] == 'Gene':
                f = '\t'.join(s[:-1])
                X_test[i, feature_to_index[f]] = float(s[-1])
            else:
...
def process_snps(sample_id, var_list, distance_matrix, indel_cost):
    ...
    for var in var_list:
        ...
        if s[0] == 'Gene' and (s[4] == '*' or s[3] == '*' or s[5] == 'FS'):
            ...
            if s[0] == 'Gene':
                if s[5] == 'snp':
                    res.add(s[0] + '\t' + s[1] + '\t' + s[2] + '\t' + str(distance_matrix[(s[3], s[4])]) + '\tsnp')
                    if merge_all_mut_in_gene:
                        ...
                        if d is None:
                            ...
                            gene_to_dist[s[0] + '\t' + s[1]] = np.sqrt(d*d + c*c)
                else:
                    res.add(var)
                    if merge_all_mut_in_gene:
                        d = gene_to_dist.get(s[0] + '\t' + s[1])
...
def main():
    ...
    tasks = parallel(delayed(split_dataset)(drug, pheno, sample_to_snps, set_name_to_list['Walker_subset'])
    ...
    with open(out_path, 'a') as f:
        ...
        f.write('LR_l1 C1 transformed extended feature set filtered mc3\n\n')
