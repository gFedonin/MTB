...
def read_coverage(sample_id):
    """
    Deprecated.
    Reads coverage files of specific format:
    header
    coverage_threshold\tpercentile,\%\tlower_percentile_val\tmedian\tupper_pesentile_val
    header2
    list of 1-based coords of intervals with coverage >= threshold

    :param sample_id: sample id
    :return: sample_id, list of covered intervals, upper_pesentile_val
    """
    coverage = []
...
def sample_cov(sample_id, all_snps, coverage):
    """
    Computes the fraction of coordinates of variants having good coverage in given sample
    :param sample_id: id of the sample
    :param all_snps: list of all variant positions
    :param coverage: list of (start, end) of intervals with good coverage
    :return: sample_id, fraction of covered variants
    """
...
def compute_snp_cov(variant_list, coverage):
    """
    Checks which of given list of variants are covered
    :param variant_list: list of variants to test
    :param coverage: list of (start, end) intervals of good coverage
    :return: numpy array x, x[i] = 1 if i-th variant from all_snps is covered, 0 if not
    """
    snp_cov = np.zeros(len(variant_list), dtype=int)
    lo = 0
    for start, end in coverage:
        s = bisect_left(variant_list, start, lo=lo)
        if s < len(variant_list):
            e = bisect_right(variant_list, end, lo=s + 1)
            # print(str(start) + ' ' + str(end) + ' ' + str(s) + ' ' + str(e))
            snp_cov[s:e] = 1
            lo = e
        else:
