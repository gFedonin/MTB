...
# out_path_gatk = data_path + 'snps/combined_nocoll/'
out_path_gatk = data_path + 'walker18/snps/gatk/'
# out_path_gatk = data_path + 'missing/snps/gatk/'
# out_path_gatk = data_path + 'coll18/snps/gatk/'
# out_path_gatk = data_path + 'snps/gatk_combined/'
# out_path_gatk = data_path + 'snps/gatk_bwa_mem_rev/'
# path_to_merged_vcf = out_path_gatk + 'merged_force_active_no_opt.vcf'
# path_to_merged_vcf = out_path_gatk + 'merged_bwa.vcf'
# path_to_merged_vcf = out_path_gatk + 'merged_bwa_combined3.vcf'
path_to_merged_vcf = out_path_gatk + 'merged_bwa_trimmed12.vcf'
# path_to_merged_vcf = out_path_gatk + 'merged_bwa_missing.vcf'
...
path_to_ids = data_path + 'walker18/trimming2.list'
# path_to_ids = data_path + 'missing/missing_trimmed.list'
# path_to_ids = data_path + 'snps/gatk_combined/combined_ids_small.list'
# path_to_ids = data_path + 'snps/gatk_combined/combined_ids.list'
# path_to_ids = out_path_gatk + 'combined_ids1.list'
# path_to_ids = out_path_gatk + 'combined_no_coll3.list'
# out_path_vcf = out_path_gatk + 'vcf_split_gatk_fa_no_opt/'
# out_path_vcf = out_path_gatk + 'vcf_split_bwa_combined3/'
out_path_vcf = out_path_gatk + 'vcf_split_bwa_trimming2/'
# out_path_vcf = out_path_gatk + 'vcf_split_bwa/'
...
def extract_sample_gatk(sample_id, out_path_vcf, big_vcf):
    if not exists(out_path_vcf + sample_id + '.vcf'):
        call(path_to_GATK + ' --java-options "-Xmx1g -Xms1g" SelectVariants --exclude-non-variants true ' +
               ...
               sample_id + ' -V ' + big_vcf + ' -O ' + out_path_vcf + sample_id + '.vcf', shell=True)
        return 1
    return 0
...
def split_gatk():
    ...
    tasks = Parallel(n_jobs=32)(delayed(extract_sample_gatk)(sample_id, out_path_vcf,  path_to_merged_vcf)
