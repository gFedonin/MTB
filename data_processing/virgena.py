...
path_to_virgena = '/export/home/fedonin/VirGenA1.4.jar'
...
path_to_config = '/export/home/fedonin/MTB/config_mtb.xml'

virgena_out_path = data_path + 'virgena_k15/'
mummer_out_path = data_path + 'mummer/dnadiff_virgena_k15/'
mummer_log_path = data_path + 'mummer/log_virgena_k15/'
mummer_out_path_snps = data_path + 'mummer/snps_virgena_k15/'
final_raw_snps_path = data_path + 'snps/virgena_mummer_raw_ld_mum4_k15/'

out_path_bam = data_path + 'virgena_bam_k15/'


def run_virgena_assembly():
    if not exists(virgena_out_path):
        makedirs(virgena_out_path)
    sample_ids = [l.strip() for l in open(path_to_list).readlines()]
    for sample_id in sample_ids:
        if exists(virgena_out_path + '/' + sample_id + '/assembly.fasta'):
            continue
        if not exists(virgena_out_path + sample_id):
            makedirs(virgena_out_path + sample_id)
        with open(virgena_out_path + sample_id + '/config.xml', 'w') as f:
            for l in open(path_to_config).readlines():
                if '<pathToReads1>' in l:
                    f.write('<pathToReads1>')
                    f.write(path_to_samples + sample_id + '/' + sample_id + '_R1.fastq.gz')
                    f.write('</pathToReads1>\n')
                elif '<pathToReads2>' in l:
                    f.write('<pathToReads2>')
                    f.write(path_to_samples + sample_id + '/' + sample_id + '_R2.fastq.gz')
                    f.write('</pathToReads2>\n')
                elif '<OutPath>' in l:
                    f.write('<OutPath>')
                    f.write(virgena_out_path + sample_id + '/')
                    f.write('</OutPath>\n')
                else:
                    f.write(l)
        check_call('java -Xmx24g -Xms4g -jar ' + path_to_virgena + ' assemble -c ' + virgena_out_path + sample_id + '/config.xml', shell=True)


def run_virgena_mapper():
    if not exists(virgena_out_path):
        makedirs(virgena_out_path)
    sample_ids = [l.strip() for l in open(path_to_list).readlines()]
    for sample_id in sample_ids:
        ...
        with open(virgena_out_path + sample_id + '/config.xml', 'w') as f:
            for l in open(path_to_config).readlines():
                if '<pathToReads1>' in l:
                    ...
                    f.write(l)
        check_call('java -Xmx24g -Xms4g -jar ' + path_to_virgena + ' map -c ' + virgena_out_path + sample_id + '/config.xml', shell=True)
...
def run_mummer(sample_id):
    ...
    if not exists(mummer_out_path + sample_id + '/' + sample_id + '.snps'):
        contigs = [l for l in open(virgena_out_path + '/' + sample_id + '/assembly.fasta').readlines()]
        if len(contigs) == 0:
            print('no contigs for ' + sample_id)
            return 1
        status = check_call(path_to_mummer + 'dnadiff  --prefix=' + mummer_out_path + sample_id + '/' + sample_id + ' ' +
                            path_to_ref + ' ' + virgena_out_path + '/' + sample_id + '/assembly.fasta > ' + mummer_log_path +
...
def mass_mummer():
    ...
    tasks = Parallel(n_jobs=-1)(delayed(run_mummer)(l) for l in listdir(virgena_out_path))
    c = 0
    for task in tasks:
        c += task
...
def parse_mummer(sample_id):
    contigs = [record for record in SeqIO.parse(virgena_out_path + '/' + sample_id + '/assembly.fasta', 'fasta')]
    if len(contigs) == 0:
        return 0
