...
path_to_perga = '/export/home/fedonin/perga/bin/perga'
...
perga_out_path = data_path + 'perga/'
mummer_out_path = data_path + 'mummer/dnadiff_perga/'
mummer_log_path = data_path + 'mummer/log_perga/'
mummer_out_path_snps = data_path + 'mummer/snps_perga/'
final_raw_snps_path = data_path + 'snps/perga_mummer_raw_ld_mum4/'

out_path_bam = data_path + 'perga_bam/'


def run_prega(sample_id):
    # if exists(perga_out_path + '/' + sample_id + '/contigs.fasta'):
    if not exists(perga_out_path + sample_id):
        makedirs(perga_out_path + sample_id)
    check_call(path_to_perga + ' all -p 1 -f ' + path_to_samples + sample_id + '/' + sample_id +
               '_R1.fastq.gz ' + path_to_samples + sample_id + '/' + sample_id + '_R2.fastq.gz -d ' +
               perga_out_path + sample_id + '/', shell=True)
    return 1


def run_perga_assembly():
    if not exists(perga_out_path):
        makedirs(perga_out_path)
    sample_ids = [l.strip() for l in open(path_to_list).readlines()]
    tasks = Parallel(n_jobs=-1)(delayed(run_prega)(sample_id) for sample_id in sample_ids)
    c = 0
    for task in tasks:
        c += task
...
def run_mummer(sample_id):
    ...
    if not exists(mummer_out_path + sample_id + '/' + sample_id + '.snps'):
        contigs = [l for l in open(perga_out_path + '/' + sample_id + '/contigs.fa').readlines()]
        if len(contigs) == 0:
            print('no contigs for ' + sample_id)
            return 1
        status = check_call(path_to_mummer + 'dnadiff  --prefix=' + mummer_out_path + sample_id + '/' + sample_id + ' ' +
                            path_to_ref + ' ' + perga_out_path + '/' + sample_id + '/contigs.fa > ' + mummer_log_path +
...
def mass_mummer():
    ...
    tasks = Parallel(n_jobs=-1)(delayed(run_mummer)(l) for l in listdir(perga_out_path))
    c = 0
    for task in tasks:
        c += task
    print('%d samples processed' % c)
...
def parse_mummer(sample_id):
    contigs = [record for record in SeqIO.parse(perga_out_path + '/' + sample_id + '/contigs.fa', 'fasta')]
    if len(contigs) == 0:
        return 0
