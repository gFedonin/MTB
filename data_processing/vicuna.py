...
from os.path import exists, pathsep, defpath
...
path_to_vicuna = "/export/home/fedonin/VICUNA_v1.3/"
...
path_to_config = path_to_vicuna + "config/vicuna_config.txt"

vicuna_out_path = data_path + 'vicuna/'
mummer_out_path = data_path + 'mummer/dnadiff_vicuna/'
mummer_log_path = data_path + 'mummer/log_vicuna/'
mummer_out_path_snps = data_path + 'mummer/snps_vicuna/'
final_raw_snps_path = data_path + 'snps/vicuna_mummer_raw_ld_mum4/'

out_path_bam = data_path + 'vicuna_bam_mummer/'

thread_num = '32'


def run_vicuna_assembly():
    if not exists(vicuna_out_path):
        makedirs(vicuna_out_path)
    sample_ids = [l.strip() for l in open(path_to_list).readlines()]
    for sample_id in sample_ids:
        if exists(vicuna_out_path + '/' + sample_id + '/contig.fasta'):
            continue
        if not exists(vicuna_out_path + sample_id):
            makedirs(vicuna_out_path + sample_id)
        check_call('pigz -d -c -p ' + thread_num + ' ' + path_to_samples + sample_id + '/' + sample_id +
                   '_R1.fastq.gz > ' + vicuna_out_path + sample_id + '/' + sample_id + '_R1.fastq', shell=True)
        check_call('pigz -d -c -p ' + thread_num + ' ' + path_to_samples + sample_id + '/' + sample_id +
                   '_R2.fastq.gz > ' + vicuna_out_path + sample_id + '/' + sample_id + '_R2.fastq', shell=True)
        with open(vicuna_out_path + sample_id + "/vicuna_config.txt", 'w') as f:
            for l in open(path_to_config).readlines():
                if 'outputDIR' in l:
                    f.write("outputDIR\t" + vicuna_out_path + sample_id + "/\n")
                elif 'pFqDir'in l:
                    f.write("pFqDir\t" + vicuna_out_path + sample_id + '/\n')
                elif 'MSAFileName' in l:
                    if l[0:1] == '//':
                        f.write(l)
                    else:
                        f.write('//' + l)
                    ...
                    f.write(l)
        path_var = pathsep.join((environ.get('LD_LIBRARY_PATH', defpath), '/export/home/fedonin/ncbi_tools/lib'))
        env = dict(environ, LD_LIBRARY_PATH=path_var, OMP_NUM_THREADS=thread_num)
        check_call(path_to_vicuna + "bin/vicuna-omp-v1.0 " + vicuna_out_path + sample_id + "/vicuna_config.txt > " +
                   vicuna_out_path + sample_id + '/' + sample_id + '.log', shell=True, env=env)
        check_call('rm ' + vicuna_out_path + sample_id + '/' + sample_id + '_R1.fastq')
        check_call('rm ' + vicuna_out_path + sample_id + '/' + sample_id + '_R2.fastq')
...
def run_mummer(sample_id):
    ...
    if not exists(mummer_out_path + sample_id + '/' + sample_id + '.snps'):
        contigs = [l for l in open(vicuna_out_path + '/' + sample_id + '/assembly.fasta').readlines()]
        if len(contigs) == 0:
            print('no contigs for ' + sample_id)
            return 1
        status = check_call(path_to_mummer + 'dnadiff  --prefix=' + mummer_out_path + sample_id + '/' + sample_id + ' ' +
                            path_to_ref + ' ' + vicuna_out_path + '/' + sample_id + '/assembly.fasta > ' + mummer_log_path +
...
def mass_mummer():
    ...
    tasks = Parallel(n_jobs=-1)(delayed(run_mummer)(l) for l in listdir(vicuna_out_path))
    c = 0
    for task in tasks:
        c += task
...
def parse_mummer(sample_id):
    contigs = [record for record in SeqIO.parse(vicuna_out_path + '/' + sample_id + '/assembly.fasta', 'fasta')]
    if len(contigs) == 0:
        return 0
