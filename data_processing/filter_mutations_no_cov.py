...
out_path = data_path + 'snps/raw_freebayes_mqm_std3_mqm30_no_highcov_no_repeats_no_cov/'
...
def filter_variants(sample_id, variants, filter_intervals, snp_only):
    """
    ...
    :return: sample_id, list of tuples (pos, alt, type) representing variants for this sample
    """
    ...
    for pos, alt, type, cov, qual, mqm in variants:
        if qual >= qual_threshold and cov >= variant_cov_threshold:
            ...
            if inside_filtered_interval:
                continue
            filtered_variants.add((pos, alt, type))
    filtered_variants = list(filtered_variants)
    filtered_variants.sort(key=lambda x: x[0])
    return sample_id, filtered_variants


def filter_sample_list(sample_to_variants, filter_intervals, snp_only):
    sample_to_vars = {}
    for sample_id, variants in sample_to_variants.items():
        sample_id, filtered_variants = filter_variants(sample_id, variants, filter_intervals, snp_only)
...
def read_all_data(sample_ids):
    ...
    tasks = Parallel(n_jobs=thread_num)(delayed(filter_sample_list)(sample_vars,
                filter_intervals, snp_only) for sample_vars in sample_lists.values())
    ...
    for sample_id, filtered_variants in sample_to_variants.items():
        for pos, alt, type in filtered_variants:
            all_snp_pos.add(pos)
    pos_list = list(all_snp_pos)
    pos_list.sort()

    print('data reading done')
    print('%d total variants after filtering' % len(all_snp_pos))
    return sample_to_variants, pos_list
...
def main():
    ...
    sample_to_variants, all_filtered_variant_poses = read_all_data(sample_ids)
